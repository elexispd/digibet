...
class UserAllRecordDeleteJob implements ShouldQueue
{
    ...
    public function handle(): void
    {
        ...
        DB::table('bet_invest_logs')->where('user_id', $this->user->id)->delete();
        DB::table('bet_invests')->where('user_id', $this->user->id)->delete();
        DB::table('referral_bonuses')->where('to_user_id', $this->user->id)->delete();
