...
class PayoutController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $payouts = Payout::with('user')->where('status', '!=', 0)
    ...
    public function confirmPayout(Request $request, $trx_id)
    {
        ...
        if ($request->isMethod('get')) {
            ...
            BasicService::makeTransaction($payout->user, $payout->net_amount_in_base_currency, $payout->charge_in_base_currency, '-',
                $payout->trx_id, 'Amount debited for payout', $payout->id, Payout::class);
